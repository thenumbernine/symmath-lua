...
-- todo move Visitor.__call's arga param out to simplify()
...
-- allows us suffix/prefix operationd to the entire recursive process
-- just like ToString. TODO merge. this means replacing all Visitor child class self() calld with self:apply()
function Eval:apply(expr, ...)
	-- call the visitor to evaluate
	return Eval.super.__call(self, expr, ...)
end
