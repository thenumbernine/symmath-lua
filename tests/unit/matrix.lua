...
require 'symmath.tests.unit.unit'(env, 'tests/unit/matrix')
...
exec[[printbr((Array(1,2) + Array(3,4))())]]
exec[[simplifyAssertEq((Array(1,2) + Array(3,4))(), Array(4,6))]]

-- numeric example

exec[[printbr(Matrix({1,2},{3,4}))]]
exec[[printbr(Matrix({1,2},{3,4}):inverse())]]
exec[[simplifyAssertEq(Matrix({1,2},{3,4}):inverse(), Matrix({-2,1},{frac(3,2),-frac(1,2)}))]]

-- 2D variable example

exec[[printbr(Matrix({a,b},{c,d}))]]
exec[[printbr(Matrix({a,b},{c,d}):inverse())]]
exec[[simplifyAssertEq(Matrix({a,b},{c,d}):inverse(), Matrix( {frac(d, a*d-b*c), frac(-b, a*d-b*c)}, {frac(-c, a*d-b*c), frac(a, a*d-b*c)} ))]]
...
exec[[simplifyAssertEq(Matrix({1,0,0,t_x},{0,1,0,t_y},{0,0,1,t_z},{0,0,0,1}):inverse(), Matrix({1,0,0,-t_x},{0,1,0,-t_y},{0,0,1,-t_z},{0,0,0,1}))]]
...
exec[[printbr(m * mInv)]]
exec[[printbr((m * mInv)())]]
exec[[simplifyAssertEq( (m*mInv)(), Matrix({1,0},{0,1}) )]]
...
exec[[simplifyAssertNe(m1, m2)]]

exec[[printbr((m1*m2):eq(m2*m1))]]
exec[[simplifyAssertNe(m1*m2, m2*m1)]]

exec[[printbr((m1*m2):eq(m1*m2))]]
exec[[simplifyAssertEq(m1*m2, m1*m2)]]
...
exec[[printbr(((Matrix({a},{b}) + Matrix({c},{d})) / t)())]]
exec[[simplifyAssertEq( ((Matrix({a},{b}) + Matrix({c},{d})) / t)(), Matrix({frac(a+c,t)}, {frac(b+d,t)}) )]]
