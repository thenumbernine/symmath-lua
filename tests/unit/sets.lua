...
x = set.RealDomain(-1,1,true,true):var'x'
assert(set.real:contains(asin(x)))
assert(set.real:contains(acos(x)))

x = set.RealDomain(1,math.huge,true,false):var'x'
assert(not set.real:contains(acos(x)))	-- TODO eventually return uncertain / touches but not contains

x = set.RealDomain(1,math.huge,false,false):var'x'
assert(not set.real:contains(acos(x)))	-- TODO return definitely false
