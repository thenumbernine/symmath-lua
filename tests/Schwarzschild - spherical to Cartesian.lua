...
local dsSq = (-(1-R/r))*dt^2 + 1/(1-R/r)*dr^2 + r^2*dtheta^2 + r^2*sin(theta)^2*dphi^2
printbr'Spherical representation:'
printbr((var'ds'^2):eq(dsSq))
dsSq = dsSq
	:replace(dr, (x*dx + y*dy + z*dz) / r)
	:replace(dtheta, (x*z*dx + y*z*dy - (x^2 + y^2)*dz) / (r^2*sqrt(x^2 + y^2)))
	:replace(dphi, (-y*dx + x*dy) / (x^2 + y^2))
	
	:replace(sin(theta), sqrt(x^2 + y^2)/r)
...
local function rapply(expr)
	if op.div.is(expr) then return expr end
	if op.pow.is(expr) then return expr:expand() end
